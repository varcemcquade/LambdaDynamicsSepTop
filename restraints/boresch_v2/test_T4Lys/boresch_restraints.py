...
n_prot = ref_prot_select.n_atoms
...
for i, complex in enumerate(complexes[1:], start=1):
    ...
    # Remap ligand indices for the merged universe
    # In merged universe: protein atoms are 0 to n_prot-1, ligand starts at n_prot
    ligand_indices_original = ligand_select.indices
    orig_to_local = {idx: j for j, idx in enumerate(ligand_indices_original)}
    lig_remapped = [n_prot + orig_to_local[idx] for idx in lig_atoms]

    local_candidates = []

    for t in candidate_triplets:
        if boresch_prot.conditions_met(new_u, lig_remapped, t):
