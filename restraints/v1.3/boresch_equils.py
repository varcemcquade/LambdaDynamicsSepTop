...
def compute_equils(u, lig_names, prot_triplet, lig_segid, box_size=84):
    """
    Compute equilibrium values for the six Boresch restraint terms.

    The six Boresch degrees of freedom are:
      - d      : distance L1–P1  (Angstroms)
      - thetaA : angle    L1–P1–P2  (degrees)
      - thetaB : angle    L2–L1–P1  (degrees)
      - phiA   : dihedral L1–P1–P2–P3  (degrees)
      - phiB   : dihedral L2–L1–P1–P2  (degrees)
      - phiC   : dihedral L3–L2–L1–P1  (degrees)

    :param u:            MDAnalysis Universe of the complex.  ``u.dimensions``
                         should already be set before calling.
    :param lig_names:    Tuple/list (l1_name, l2_name, l3_name) — atom names
                         within the ligand segment.
    :param prot_triplet: [p1, p2, p3] with *1-based* protein atom indices.
    :param lig_segid:    Ligand segment ID (e.g. "HETA").
    :param box_size:     Cubic box side length in Angstroms (used to set box
                         if not already set on ``u``).
    ...
    # prot_triplet is 1-based → convert to 0-based for u.atoms indexing
    p1_idx = prot_triplet[0] - 1
    p2_idx = prot_triplet[1] - 1
    p3_idx = prot_triplet[2] - 1
